...
async def run_comparison():
    ...
    protected_results = await run_attacks(
        protected_agent, protected_runner, delay_seconds=0
    )
...
def print_comparison(unprotected, protected):
    ...
    for i, (u, p) in enumerate(zip(unprotected, protected), 1):
        u_status = (
            "ERROR" if u.get("error")
            else ("BLOCKED" if u.get("blocked") else "LEAKED")
        )
        p_status = (
            "ERROR" if p.get("error")
            else ("BLOCKED" if p.get("blocked") else "LEAKED")
        )
...
class SecurityTestPipeline:
    ...
    async def run_all(self, attacks: list = None, delay_seconds: float = 7.0) -> list:
        ...
        for index, attack in enumerate(attacks):
            ...
            if delay_seconds and index < len(attacks) - 1:
